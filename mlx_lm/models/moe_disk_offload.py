...
_LAYER_RE = re.compile(r"(?:^|\.)layers\.(\d+)\.mlp\.switch_mlp\.")
...
def is_expert_tensor_name(name: str, layers=None) -> bool:
    is_expert = ".mlp.switch_mlp." in name and name.rsplit(".", 1)[-1] in {
        "weight",
        "scales",
        "biases",
        "bias",
...
class DiskBackedSwitchGLU(nn.Module):
    def __init__(
        ...
        self.gate_proj = DiskBackedSwitchLinear(
            input_dims, hidden_dims, num_experts, f"{prefix}.gate_proj", store, bias=bias
        )
        self.up_proj = DiskBackedSwitchLinear(
            input_dims, hidden_dims, num_experts, f"{prefix}.up_proj", store, bias=bias
        )
        self.down_proj = DiskBackedSwitchLinear(
            hidden_dims, input_dims, num_experts, f"{prefix}.down_proj", store, bias=bias
        )
